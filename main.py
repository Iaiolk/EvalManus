...
async def main():
    ...
    try:
        ...
        prompt = """搜索引擎返回结果的准确性一定程度上会影响到用户的使用体验，现在需要你判断搜索引擎中返回的搜索结果中是否有准确性问题。请首先调用有关工具读取task.txt文件可以获取搜索query和用户搜索行为历史session_text。然后判断搜索query是否属于{事实性问题、数据查询、历史事件}等客观性很强的问题，如果是，则任务该query应该有准确答案，否则则没有准确答案，你可以自行设定详细的判断标准。对于没有准确答案的query，不需要进行后续分析；对于有准确答案的query，你需要接着调用model_search工具获取大模型提供的标准答案，session_text中搜索引擎返回的结果中可能存在多种问题，包括但不限于[前后多个提供结果不一致，与标准答案不一致，时效问题]等多种问题，你可以自行决定具体的判定标准，最后打一个评价分数。把你的判断标准、判断原因和判断结果写入一个结果文件cls_res.json中。"""
        if not prompt.strip():
