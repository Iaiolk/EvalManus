...
class FileOperator(Protocol):
    ...
    async def run_command(
        ...
        """运行shell命令并返回(return_code, stdout, stderr)。"""
        ...
    ...
    async def run_command(
        ...
        try:
            ...
            try:
                ...
                pass
            raise TimeoutError(f"命令'{cmd}'在{timeout}秒后超时") from exc
...
class SandboxFileOperator(FileOperator):
    ...
    async def run_command(
        ...
        try:
            ...
            return (
                0,  # 始终返回0，因为我们没有来自沙箱的明确返回码
                stdout,
                "",  # 当前沙箱实现中没有stderr捕获
            )
        except TimeoutError as exc:
            raise TimeoutError(f"命令'{cmd}'在沙箱中{timeout}秒后超时") from exc
